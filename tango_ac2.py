...
# Load images
ASSET_PATH = "/Users/mgrsuraz/Downloads/PlayTango/images/"
...
def get_domains(grid, locked):
    domains = {}
    for i in range(GRID_SIZE):
        for j in range(GRID_SIZE):
            if (i, j) in locked or grid[i][j] != 0:
                continue
            domains[(i, j)] = [1, 2]  # sun or moon
    return domains
...
def revise(x, y, domains, grid):
    ...
    for vx in domains[x]:
        ...
        for vy in domains[y]:
            grid[y] = vy
            if not check_triples(grid) and not check_equal_counts(grid) and not check_constraints(grid):
                satisfies = True
                break
        if satisfies:
            new_domain.append(vx)
    if set(new_domain) != set(domains[x]):
...
def ac3(domains, grid):
    queue = [(x, y) for x in domains for y in neighbors(x) if y in domains]
    while queue:
        x, y = queue.pop(0)
        if revise(x, y, domains, grid):
            if not domains[x]:
                return False
...
def a_star_solver(start_grid, locked):
    domains = get_domains(start_grid, locked)
    if not ac3(domains, start_grid):
